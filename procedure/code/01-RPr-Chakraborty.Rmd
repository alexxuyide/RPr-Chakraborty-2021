...
### Alex needs to change this table 
```{r missing data}
...
print(
  paste(
    "Run time:",
    round(difftime(Sys.time(), start_time_05, units = "mins"), 2),
    "minutes"
  ),
  quote = FALSE
)
...
saveRDS(covid_kulldorff_05, file = here("data", "derived", "public", paste0("covid_kulldorff_05_", Sys.Date(), ".RDS")) )
...
covid_kulldorff_05$most.likely.cluster
print(
  paste0(
    "Number of Secondary clusters: ",
    length(covid_kulldorff_05$secondary.clusters)
  ),
  quote = FALSE
)

print("Most likely cluster 35 percent:", quote = FALSE)
...
cluster_locations_05 <- covid_kulldorff_05$most.likely.cluster$location.IDs.included

# create data frame of clusters and
# calculate the clusterID as the first (center) county FIPS code
clusters_05 <- covid_table[cluster_locations_05, "fips"] %>%
  mutate(clusterID = covid_table[[cluster_locations_05[1], "fips"]],
         likelihood = covid_kulldorff_05$most.likely.cluster$log.likelihood.ratio)

# Get a list of secondary clusters
secondary_05 <- covid_kulldorff_05$secondary.clusters

# similarly add counties in each secondary cluster to the list of clusters
for (i in secondary_05) {
  cluster_locations_05 <- i$location.IDs.included
  new_clusters <- covid_table[cluster_locations_05, "fips"] %>%
    mutate(clusterID = covid_table[[cluster_locations_05[1], "fips"]],
           likelihood = i$log.likelihood.ratio)
  clusters_05 <- clusters_05 %>% rbind(new_clusters)
}

rm(cluster_locations_05, secondary_05, i, new_clusters)
```

### Alex's deviation
Do the same thing for pop upper bound = 35%
```{r}
# list of primary cluster locations (counties)
...
clusters_015 <- covid_table[cluster_locations_015, "fips"] %>%
  mutate(clusterID = covid_table[[cluster_locations_015[1], "fips"]],
...
acs_covid_05 <- acs_covid[, 1:which(colnames(acs_covid) == "or_incidence")] %>%
  left_join(clusters_05, by = "fips") %>%
...
```{r map-clusters}
tm_spatialepi_clusters_05 <-
  tm_shape(state) +
    tm_fill("gray98") +
  tm_shape(acs_covid_05) +
...
tm_spatialepi_clusters_015 <-
  ...
  tm_polygons(col = "isCluster",
          ...
          popup.vars = c("fips", "clusterID"),
          colorNA = NULL,
          title = "SpatialEpi Kulldorff COVID-19 Clusters Pop=35%",
          border.col = "white",
          lwd = 0.2,
          border.alpha = 0.2) +
  tm_shape(state) +
