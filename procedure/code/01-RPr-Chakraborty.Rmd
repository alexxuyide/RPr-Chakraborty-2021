...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Alex Xu"
date: "`r Sys.Date()`"
...
Version 2.1 \| First Created July 7, 2021 \| Updated September, 2023
...
# Save query results
saveRDS(acs, here("data", "raw", "public", "acs.RDS"))  # here() where to save, saveRDS()
...
rm(acs, acs_derived, covid) # to remove these data
```
...
horizontal <- t(filter(acs_covid, is.na(bpov_pct))) %>% st_drop_geometry() %>%kable()

names(horizontal)[1] <- "Values"
...
# How does the legend come up with 5 quantiles?
```
...
### Alex's modification plan

**Changing the population upper bound in Kulldorff spatial scan**

In my modification study, I chose to change the population threshold of the Kulldorff scan statistics of SpatialEpi and see how it would affect the extent of the clusters.
At default condition, under which the original study was conducted, the threshold `pop.upper.bound` was kept as 50%.
This parameter indicates the largest percentage of population that is within the cluster when the computer determining the extent of the cluster that could be centered on every county in the United States.
Operating under the default condition, the computer is going to perform many unnecessary calculations, because it is unreasonable to have really large cluster of COVID incidence.
Additionally, I assumed that the large cluster of the South that was generated by the Kulldorf scan under default condition could be broken into smaller and more reasonable clusters by lowering the `pop.upper.bound` threshold.
I will change the `pop.upper.bound` parameter to 15%, 10%, and 5%. After the cluster results are generated, I will also calculate the relative risks of the clusters under different scenarios and compare the cluster relative risk to the local relative risk of the counties. 
...
rm(start_time )
#covid_geo, expected.cases,


```


```{r SpatialEpi-Kulldorff_015, eval = FALSE, fig.width=4, fig.height=4}
start_time_015 <- Sys.time()
covid_geo <- covid_table %>%
  select(x, y) %>%
  latlong2grid()
# latlong2grid creates approximate equidistant cylindrical grid
...
print(
  paste(
    "Run time:",
    round(difftime(Sys.time(), start_time_015, units = "mins"), 2),
    "minutes"
  ),
  quote = FALSE
)
...
```{r SpatialEpi-Kulldorff_010, eval = FALSE, fig.width=4, fig.height=4}
start_time_010 <- Sys.time()
covid_geo <- covid_table %>%
  select(x, y) %>%
  latlong2grid()
# latlong2grid creates approximate equidistant cylindrical grid
...
print(
  paste(
    "Run time:",
    round(difftime(Sys.time(), start_time_010, units = "mins"), 2),
    "minutes"
  ),
  quote = FALSE
)
...
```{r SpatialEpi-Kulldorff_005, eval = FALSE, fig.width=4, fig.height=4}
start_time_005 <- Sys.time()
covid_geo <- covid_table %>%
  select(x, y) %>%
  latlong2grid()
# latlong2grid creates approximate equidistant cylindrical grid
...
print(
  paste(
    "Run time:",
    round(difftime(Sys.time(), start_time_005, units = "mins"), 2),
    "minutes"
  ),
  quote = FALSE
)
...
```{r SpatialEpi-Kulldorff-save, eval = FALSE, fig.width=4, fig.height=4}

# {save results in a file appended with the current date}

saveRDS(covid_kulldorff, file = here("data", "derived", "public", paste0("covid_kulldorff_", Sys.Date(), ".RDS")) )

saveRDS(covid_kulldorff_015, file = here("data", "derived", "public", paste0("covid_kulldorff_015_", Sys.Date(), ".RDS")) )

saveRDS(covid_kulldorff_010, file = here("data", "derived", "public", paste0("covid_kulldorff_010_", Sys.Date(), ".RDS")) )

saveRDS(covid_kulldorff_005, file = here("data", "derived", "public", paste0("covid_kulldorff_005_", Sys.Date(), ".RDS")) )
...
covid_kulldorff_015  <- readRDS(
  here("data", "derived", "public", "covid_kulldorff_015_2023-10-02.RDS")
)

covid_kulldorff_010  <- readRDS(
  here("data", "derived", "public", "covid_kulldorff_010_2023-10-02.RDS")
)

covid_kulldorff_005  <- readRDS(
  here("data", "derived", "public", "covid_kulldorff_005_2023-10-02.RDS")
)
```
...
```{r report-Kulldorff}
print("Most likely cluster 50 percent:", quote = FALSE)
...
print("Most likely cluster 15 percent:", quote = FALSE)
covid_kulldorff_015$most.likely.cluster
print(
  paste0(
    "Number of Secondary clusters: ",
    length(covid_kulldorff_015$secondary.clusters)
  ),
  quote = FALSE
)

print("Most likely cluster 10 percent:", quote = FALSE)
covid_kulldorff_010$most.likely.cluster
print(
  paste0(
    "Number of Secondary clusters: ",
    length(covid_kulldorff_010$secondary.clusters)
  ),
  quote = FALSE
)

print("Most likely cluster 5 percent:", quote = FALSE)
covid_kulldorff_005$most.likely.cluster
print(
  paste0(
    "Number of Secondary clusters: ",
    length(covid_kulldorff_005$secondary.clusters)
  ),
  quote = FALSE
)
```
...
```{r assign-cluster-IDs_15, message = FALSE}
# list of primary cluster locations (counties)
cluster_locations_015 <- covid_kulldorff_015$most.likely.cluster$location.IDs.included

# create data frame of clusters and
# calculate the clusterID as the first (center) county FIPS code
clusters_015 <- covid_table[cluster_locations_015, "fips"] %>%
  mutate(clusterID_015 = covid_table[[cluster_locations_015[1], "fips"]],
         likelihood = covid_kulldorff_015$most.likely.cluster$log.likelihood.ratio)

# Get a list of secondary clusters
secondary_015 <- covid_kulldorff_015$secondary.clusters

# similarly add counties in each secondary cluster to the list of clusters
for (i in secondary_015) {
  cluster_locations_015 <- i$location.IDs.included
  new_clusters <- covid_table[cluster_locations_015, "fips"] %>%
    mutate(clusterID_015 = covid_table[[cluster_locations_015[1], "fips"]],
           likelihood = i$log.likelihood.ratio)
  clusters_015 <- clusters_015 %>% rbind(new_clusters)
}

rm(cluster_locations_015, secondary_015, i, new_clusters)
```



```{r assign-cluster-IDs_10, message = FALSE}
# list of primary cluster locations (counties)
cluster_locations_010 <- covid_kulldorff_010$most.likely.cluster$location.IDs.included

# create data frame of clusters and
# calculate the clusterID as the first (center) county FIPS code
clusters_010 <- covid_table[cluster_locations_010, "fips"] %>%
  mutate(clusterID_010 = covid_table[[cluster_locations_010[1], "fips"]],
         likelihood = covid_kulldorff_010$most.likely.cluster$log.likelihood.ratio)

# Get a list of secondary clusters
secondary_010 <- covid_kulldorff_010$secondary.clusters

# similarly add counties in each secondary cluster to the list of clusters
for (i in secondary_010) {
  cluster_locations_010 <- i$location.IDs.included
  new_clusters <- covid_table[cluster_locations_010, "fips"] %>%
    mutate(clusterID_010 = covid_table[[cluster_locations_010[1], "fips"]],
           likelihood = i$log.likelihood.ratio)
  clusters_010 <- clusters_010 %>% rbind(new_clusters)
}

rm(cluster_locations_010, secondary_010, i, new_clusters)
```


```{r assign-cluster-IDs_5, message = FALSE}
# list of primary cluster locations (counties)
cluster_locations_005 <- covid_kulldorff_005$most.likely.cluster$location.IDs.included

# create data frame of clusters and
# calculate the clusterID as the first (center) county FIPS code
clusters_005 <- covid_table[cluster_locations_005, "fips"] %>%
  mutate(clusterID_005 = covid_table[[cluster_locations_005[1], "fips"]],
         likelihood = covid_kulldorff_005$most.likely.cluster$log.likelihood.ratio)

# Get a list of secondary clusters
secondary_005 <- covid_kulldorff_005$secondary.clusters

# similarly add counties in each secondary cluster to the list of clusters
for (i in secondary_005) {
  cluster_locations_005 <- i$location.IDs.included
  new_clusters <- covid_table[cluster_locations_005, "fips"] %>%
    mutate(clusterID_005 = covid_table[[cluster_locations_005[1], "fips"]],
           likelihood = i$log.likelihood.ratio)
  clusters_005 <- clusters_005 %>% rbind(new_clusters)
}

rm(cluster_locations_005, secondary_005, i, new_clusters)
```
...
acs_covid_015 <- acs_covid[, 1:which(colnames(acs_covid) == "or_incidence")] %>%
  left_join(clusters_015, by = "fips") %>%
  mutate(isCluster = case_when(
    clusterID_015 == fips ~ "center of cluster",
    !is.na(clusterID_015) ~ "other part of cluster",
    .default = NA
  ))

acs_covid_010 <- acs_covid[, 1:which(colnames(acs_covid) == "or_incidence")] %>%
  left_join(clusters_010, by = "fips") %>%
  mutate(isCluster = case_when(
    clusterID_010 == fips ~ "center of cluster",
    !is.na(clusterID_010) ~ "other part of cluster",
    .default = NA
  ))

acs_covid_005 <- acs_covid[, 1:which(colnames(acs_covid) == "or_incidence")] %>%
  left_join(clusters_005, by = "fips") %>%
  mutate(isCluster = case_when(
    clusterID_005 == fips ~ "center of cluster",
    !is.na(clusterID_005) ~ "other part of cluster",
    .default = NA
  ))
...
tm_spatialepi_clusters  <-
  ...
  tm_polygons(col = "isCluster",
          ...
          colorNA = NULL,
          title = "SpatialEpi Kulldorff COVID-19 Clusters Pop=50%",
          border.col = "white",
          lwd = 0.2,
          border.alpha = 0.2) +
  tm_shape(state) +
...
tm_spatialepi_clusters_015 <-
  tm_shape(state) +
    tm_fill("gray98") +
  tm_shape(acs_covid_015) +
  tm_polygons(col = "isCluster",
          palette = "-Oranges",
          popup.vars = c("fips", "clusterID"),
          colorNA = NULL,
          title = "SpatialEpi Kulldorff COVID-19 Clusters Pop=15%",
          border.col = "white",
          lwd = 0.2,
          border.alpha = 0.2) +
  tm_shape(state) +
...
tm_spatialepi_clusters_010 <-
  tm_shape(state) +
    tm_fill("gray98") +
  tm_shape(acs_covid_010) +
  tm_polygons(col = "isCluster",
          palette = "-Oranges",
          popup.vars = c("fips", "clusterID"),
          colorNA = NULL,
          title = "SpatialEpi Kulldorff COVID-19 Clusters Pop=10%",
          border.col = "white",
          lwd = 0.2,
          border.alpha = 0.2) +
  tm_shape(state) +
...
tm_spatialepi_clusters_005 <-
  tm_shape(state) +
    tm_fill("gray98") +
  tm_shape(acs_covid_005) +
  tm_polygons(col = "isCluster",
          palette = "-Oranges",
          popup.vars = c("fips", "clusterID"),
          colorNA = NULL,
          title = "SpatialEpi Kulldorff COVID-19 Clusters Pop=5%",
          border.col = "white",
          lwd = 0.2,
          border.alpha = 0.2) +
  tm_shape(state) +
...
acs_covid_005 <- acs_covid_005 %>%
  group_by(clusterID_005) %>%
  mutate(
    rr_cluster = ifelse(is.na(clusterID_005), NA,
      (sum(cases) / sum(pop)) / ((total_cases - sum(cases)) / (total_pop - sum(pop)))
    )
  ) %>%
  ungroup() %>%
...
acs_covid_010 <- acs_covid_010 %>%
  group_by(clusterID_010) %>%
  mutate(
    rr_cluster = ifelse(is.na(clusterID_010), NA,
      (sum(cases) / sum(pop)) / ((total_cases - sum(cases)) / (total_pop - sum(pop)))
    )
  ) %>%
  ungroup() %>%
...
acs_covid_015 <- acs_covid_015 %>%
  group_by(clusterID_015) %>%
  mutate(
    rr_cluster = ifelse(is.na(clusterID_015), NA,
      (sum(cases) / sum(pop)) / ((total_cases - sum(cases)) / (total_pop - sum(pop)))
    )
  ) %>%
  ungroup() %>%
...
acs_covid_005 <- acs_covid_005 %>%
  mutate(
    cluster_class_005 = ifelse(is.na(clusterID_005), 1, cut(rr_cluster, breaks, labels = FALSE)),
    loc_class = cut(rr_loc, breaks, labels = FALSE)
  )

acs_covid_010 <- acs_covid_010 %>%
  mutate(
    cluster_class_010 = ifelse(is.na(clusterID_010), 1, cut(rr_cluster, breaks, labels = FALSE)),
    loc_class = cut(rr_loc, breaks, labels = FALSE)
  )

acs_covid_015 <- acs_covid_015 %>%
  mutate(
    cluster_class_015 = ifelse(is.na(clusterID_015), 1, cut(rr_cluster, breaks, labels = FALSE)),
    loc_class = cut(rr_loc, breaks, labels = FALSE)
  )
...
Next, map the cluster relative risk scores for comparison between clusters generated by different population thresholds.
...
# count the frequency of counties in each class and create labels
class_freq <- acs_covid_005%>% st_drop_geometry() %>% count(cluster_class_005)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq[1, ]$qual <- paste(class_freq[1, ]$qual, "at low risk")
class_freq[nrow(class_freq), ]$qual <- paste(class_freq[nrow(class_freq), ]$qual, "at high risk")
class_freq$label <- paste0(class_freq$cluster_class_005,
                           " (",
                           class_freq$n,
                           class_freq$qual,
                           ")")
...
# map cluster relative risk scores
tm_spatialepi_cluster_risk_class_005 <- tm_shape(acs_covid_005) +
  tm_polygons("cluster_class_005",
    title = "Cluster Relative Risk Class\n (pop threshold = 5%)",
    border.col = "white",
    border.alpha = .2,
    lwd = 0.2,
...
# count the frequency of counties in each class and create labels
class_freq <- acs_covid_010 %>% st_drop_geometry() %>% count(cluster_class_010)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq[1, ]$qual <- paste(class_freq[1, ]$qual, "at low risk")
class_freq[nrow(class_freq), ]$qual <- paste(class_freq[nrow(class_freq), ]$qual, "at high risk")
class_freq$label <- paste0(class_freq$cluster_class_010,
                           " (",
                           class_freq$n,
                           class_freq$qual,
                           ")")
...
# map cluster relative risk scores
tm_spatialepi_cluster_risk_class_010 <- tm_shape(acs_covid_010) +
  tm_polygons("cluster_class_010",
    title = "Cluster Relative Risk Class\n (pop threshold = 10%)",
    border.col = "white",
    border.alpha = .2,
    lwd = 0.2,
...
# count the frequency of counties in each class and create labels
class_freq <- acs_covid_015 %>% st_drop_geometry() %>% count(cluster_class_015)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq[1, ]$qual <- paste(class_freq[1, ]$qual, "at low risk")
class_freq[nrow(class_freq), ]$qual <- paste(class_freq[nrow(class_freq), ]$qual, "at high risk")
class_freq$label <- paste0(class_freq$cluster_class_015,
                           " (",
                           class_freq$n,
                           class_freq$qual,
                           ")")
...
# map cluster relative risk scores
tm_spatialepi_cluster_risk_class_015 <- tm_shape(acs_covid_015) +
  tm_polygons("cluster_class_015",
    title = "Cluster Relative Risk Class\n (pop threshold = 15%)",
    border.col = "white",
    border.alpha = .2,
    lwd = 0.2,
...
tm_spatialepi_cluster_risk_class_015
tm_spatialepi_cluster_risk_class_010
tm_spatialepi_cluster_risk_class_005
```

### Alex's New Map

To compare the difference between the cluster relative risk and local relative risk class, here I add a new set of maps that show the difference between the cluster relative risk class and the county relative risk class for clusters of different population thresholds.

```{r}
acs_covid <- acs_covid %>%
  mutate(local_cluster_diff = loc_class - cluster_class)

acs_covid_015 <- acs_covid_015 %>%
  mutate(local_cluster_diff = loc_class - cluster_class_015)

acs_covid_010 <- acs_covid_010 %>%
  mutate(local_cluster_diff = loc_class - cluster_class_010)

acs_covid_005 <- acs_covid_005 %>%
  mutate(local_cluster_diff = loc_class - cluster_class_005)

#default map
# count the frequency of counties in each class and create labels
class_freq <- acs_covid %>% st_drop_geometry() %>% count(local_cluster_diff)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq$label <- paste0(class_freq$local_cluster_diff,
                           " (",
                           class_freq$n,
                           class_freq$qual,
                           ")")
...
# map cluster relative risk scores
tm_spatialepi_risk_class_diff <- tm_shape(acs_covid) +
  tm_polygons("local_cluster_diff",
    title = "RR Class Difference (Local-Cluster)",
    border.col = "white",
    style = "equal",
    n = 7,
    border.alpha = .2,
...
#15% map
# count the frequency of counties in each class and create labels
class_freq <- acs_covid_015 %>% st_drop_geometry() %>% count(local_cluster_diff)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq$label <- paste0(class_freq$local_cluster_diff,
                           " (",
                           class_freq$n,
...
# map cluster relative risk scores
tm_spatialepi_risk_class_diff_015 <- tm_shape(acs_covid_015) +
  tm_polygons("local_cluster_diff",
    title = "RR Class Difference (Local-Cluster)\n (pop threshold = 15%)",
    border.col = "white",
    style = "equal",
...
#10% map
# count the frequency of counties in each class and create labels
class_freq <- acs_covid_010 %>% st_drop_geometry() %>% count(local_cluster_diff)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq$label <- paste0(class_freq$local_cluster_diff,
                           " (",
                           class_freq$n,
...
# map cluster relative risk scores
tm_spatialepi_risk_class_diff_010 <- tm_shape(acs_covid_010) +
  tm_polygons("local_cluster_diff",
    title = "RR Class Difference (Local-Cluster)\n (pop threshold = 10%)",
    border.col = "white",
    style = "equal",
...
#5% map
# count the frequency of counties in each class and create labels
class_freq <- acs_covid_005 %>% st_drop_geometry() %>% count(local_cluster_diff)
class_freq$qual <- ifelse(class_freq$n > 1, " counties", " county")
class_freq$label <- paste0(class_freq$local_cluster_diff,
                           " (",
                           class_freq$n,
...
# map cluster relative risk scores
tm_spatialepi_risk_class_diff_005 <- tm_shape(acs_covid_005) +
  tm_polygons("local_cluster_diff",
    title = "RR Class Difference (Local-Cluster)\n (pop threshold = 5%)",
    border.col = "white",
    style = "equal",
...
tm_spatialepi_risk_class_diff
tm_spatialepi_risk_class_diff_015
tm_spatialepi_risk_class_diff_010
tm_spatialepi_risk_class_diff_005
```
...
 -**Ecological fallacy**: There is a risk of ecological fallacy with this research design, whereby county-level statistics for the whole population should not be interpreted as definitive inferential proof of individual-level relationships, especially for populations comprising no more than one third of any county.
...
 -**Scale dependency**: There is a real possibility that the study findings are scale-dependent, but it is also impossible to replicate the study using a finer spatial support without also limiting the extent to one or more adjacent states with sub-county data on COVID-19.

 -**Spatial dependency**: GEE method simply uses correlation matrices to weight each county observation according to the number of observations in its cluster and the overall within-cluster correlation of standard errors.
The spatial dependence between counties in different clusters is not modelled, even if the counties are adjacent to each other - Modifiable areal unit problem: The modifiable areal unit problem (MAUP) may also be a source of uncertainty, especially when considering the variable population sizes of counties, which all carry equal weights in the analysis.

 -**Variable measurement**: There are well-known problems with the testing and reporting of COVID-19 cases across time and space during the pandemic in the United States, and there is also uncertainty in American Community Survey (ACS) data, particularly when cross-tabulating multiple demographic characteristics.
...
## Discussion from Alex: Findings from Changing the Population Threshold in Kulldorff Spatial Scan

In my modification study, I chose to modify the population threshold of the Kulldorff scan statistics in SpatialEpi and observe its impact on the extent of clusters.
In the original study and the reproduction steps, the `pop.upper.bound` threshold was maintained at the default value, which is 50%.
Initially, I assumed that the large cluster in the South was a result of the high threshold, and by lowering the `pop.upper.bound` threshold, it might break the large cluster into smaller, more reasonable clusters.

I first lowered the threshold to 35%, but I did not observe significant differences in the resulting clusters.
I calculated that the population covered by the large cluster constituted approximately 16% of the total United States population.
Therefore, I subsequently tested thresholds starting from 15%.
I reduced the threshold to 15%, 10%, and 5%, and compared the resulting clusters with those generated using the default threshold.

In the results with a 15% threshold, the large South cluster contracted slightly towards the Southeast, revealing some urban clusters, including Nashville.
The 10% and 5% thresholds resulted in the large cluster splitting into two moderately sized clusters, centered around counties in Mississippi and South Carolina.
This separation left urban counties like Atlanta, Memphis, and Miami in their own clusters.
Although these two clusters were still relatively large, lowering the threshold allowed us to distinguish urban clusters from a single, extensive cluster seen with the default threshold.

Subsequently, I calculated the relative risk for clusters at different thresholds and created a new set of maps illustrating the difference between local relative risk and cluster relative risk.
Ideally, we aimed to create clusters that neither underestimated nor overestimated the local relative risk.
These maps were used to compare clusters generated by different thresholds in terms of the number of counties showing differences between local and cluster relative risk.
The maps indicated that the 10% and 5% clusters had smaller differences compared to the 15% and default clusters.

Although lowering the threshold did not result in the same number of clusters as SaTScan in the original study, it was discovered that adjusting the population threshold from the default value led to more reasonable clusters for analysis.
...
We divide the study into three parts,

1.  Part 1: COVID incident map production
2.  Part 2: Bivariate correlation and summary statistics
3.  Part 3: Clusters and GEE models

After finishing each step, we compare our results with the author's original ones.

*Part 1: COVID incident map production*
...
In addition to the distribution of COVID cases, we also mapped the spatial distribution of disability (Figure 3).
...
The two spatial distributions help explain the negative bivariate relationship between disability rates and COVID-19 incidence in the next part.

*Part 2: Bivariate correlation and summary statistics*

The results of the second part matched perfectly with those of the original study, confirming that the reproduction uses identical original data.
...
*Part 3: Clusters Generating and GEE Models*

We noticed that Kulldorff spatial scan statistic, the tool we used for cluster generation, varied significantly between the clusters of the original study, which were generated by SaTScan, and the clusters of our reproduction and re-analysis, which are generated by SpatialEpi, with regards to selection of secondary clusters, interpretation of clusters, and relative risk scores.
Before implementing SaTScan like what was did in the original study, we used SpatialEpi first to identify clusters.
Under the clusters generated by Kulldorff spatial scan in SpatialEpi, we failed to reproduce the same configuration of GEE clusters as the original study, finding 9 more clusters than the original study and a much smaller maximum cluster of 159 counties compared to 245 counties.
After failing to reproduce the same clusters, we decide to use SaTScan, which is not open.
Fortunately, our SaTScan results exactly reproduced the author-provided SaTScan results data and the SaTScan results exactly reproduce the author-reported frequency of original GEE classes and maximum counties per class.

For the GEE Models, our results were largely consistent with Chakraborty's results, suggesting robustness to modeling decisions with regards to GEE clusters.
However, one thing to notice is that some differences occur when we tried to switch the computational environment from the study's original SPSS environment to geepack.
Fixing the data of the 13 counties with inconsistent incident rates we found at the beginning of the study does not have significant impact on the coefficient estimates.
some coefficients that remained significant and robust to changes in computational environment, data errors, and clustering criteria.
Some robust positive coefficients include Black, Native American, Non-Hispanic non-White, Below poverty level, and Female; some robust negative coefficient include white, Non-Hispanic White, Above poverty level, and Male, which are all less vulnerable communities.
